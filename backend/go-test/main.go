...
func main() {
	...
	router.POST("/upload", func(c *gin.Context) {
		...
		for _, record := range records {
			// Sanitize the TemplateType and ID to remove any invalid filename characters
			sanitizedTemplateType := strings.ReplaceAll(record.TemplateType, " ", "_")
			sanitizedID := strings.ReplaceAll(record.ID, " ", "_")

			// Create a unique filename using TemplateType and ID
			filename := fmt.Sprintf("%s/%s_%s.pdf", storagePath, sanitizedTemplateType, sanitizedID)

			// Generate the PDF
			err := generatePDF(record, filename)
			if err != nil {
				c.JSON(500, gin.H{"error": fmt.Sprintf("Failed to generate PDF for record %s: %v", record.ID, err)})
				return
			}
		}

		c.JSON(200, gin.H{"message": "PDFs generated successfully!"})
	})
